  ...
  mounted() {
    ...
    axios.all([req(httpAC)]).then(
      axios.spread((res) => {
        ...
        arrDom[0].style.backgroundImage="url("+ BannerPicInfoObj.getBannerPicImgUrl(res)+")"
        arrDom[1].style.backgroundImage="url("+ BannerPicInfoObj.getBannerPendantImgUrl(res)+")"
      })
    )
